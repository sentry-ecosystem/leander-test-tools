...
def _is_synthetic_event(event):
    """Check if an event was generated by error-generator.sentry.dev."""
    tags = event.get("tags", {})
    if isinstance(tags, dict) and tags.get("Generated By"):
        return True
    if isinstance(tags, list):
        for tag in tags:
            if isinstance(tag, (list, tuple)) and len(tag) >= 2 and tag[0] == "Generated By":
                return True
    transaction = event.get("transaction", "")
    if isinstance(transaction, str) and re.match(r"test-transaction-\d+-", transaction):
        return True
    return False


def before_send(event, hint):
    """Drop synthetic errors from error-generator.sentry.dev."""
...
def before_send_transaction(event, hint):
    """Drop synthetic transactions from error-generator.sentry.dev."""
    if _is_synthetic_event(event):
        return None
    return event
