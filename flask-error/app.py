...
def before_send(event, hint):
    """Drop synthetic events sent by error-generator.sentry.dev."""
    tags = event.get("tags", {})
    if isinstance(tags, dict):
        if tags.get("Generated By") == "error-generator.sentry.dev":
            return None
    elif isinstance(tags, list):
        for tag in tags:
            if isinstance(tag, (list, tuple)) and len(tag) == 2:
                if tag[0] == "Generated By" and tag[1] == "error-generator.sentry.dev":
                    return None
    return event
